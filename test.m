...
% %     将一个周期以及pweeks加入train_data
    ...
    test_pulse(num,1:fft_size) = (c-min(c))/(max(c)-min(c));
    test_label(num) = a{n,2};
    num = num +1;
...
%     quality = 3;
% elseif length(right_period) < 1/2*m
%     quality = 4;
% else
%     if length(point) > 3/4 * length(right_period)
%         quality = 1; 
%     else
%         quality = 2;
%     end
